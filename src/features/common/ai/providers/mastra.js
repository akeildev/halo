...
async function createMastraStream(messages, options = {}) {
    try {
            ...
            if (Array.isArray(userMessage.content)) {
                ...
                for (const content of userMessage.content) {
                    if (content.type === 'text') {
                        prompt += content.text + ' ';
                    } else if (content.type === 'image' || content.type === 'image_url') {
                        hasImage = true;
        ...
        // Debug log the exact message format being sent
        if (hasImage) {
            console.log('[Mastra] Message format being sent:', JSON.stringify(messages, null, 2));
        }
